...
export async function getAdminOverview(): Promise<GetAdminOverviewOutputType> {
	...
	const [
		...
		prisma.pack.count({ where: { createdAt: { gte: recentFrom } } }),
		prisma.topic.count({ where: { createdAt: { gte: recentFrom } } }),
		prisma.question.count({ where: { createdAt: { gte: recentFrom } } }),
		prisma.game.count({
		...
		prisma.reaction.count({ where: { createdAt: { gte: recentFrom } } }),
		prisma.click.count({ where: { createdAt: { gte: recentFrom } } }),
		prisma.playerBadgeAward.count({
